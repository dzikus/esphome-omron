...
entries = [e for e in json.load(open(db_path)) if '/components/omron/' in e['file']]
if not entries:
    sys.exit('compile database holds no omron translation unit')
...
clang-tidy -p "${work}" --header-filter='.*/components/omron/.*' "${sources[@]}" > "${report}" 2>&1 || true
...
if grep -E '/components/omron/[^:]+:[0-9]+:[0-9]+: (warning|error):' "${report}"; then
